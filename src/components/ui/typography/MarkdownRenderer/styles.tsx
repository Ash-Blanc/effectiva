...
const ThoughtBlock = ({ children, ...props }: any) => {
    const [isOpen, setIsOpen] = useState(false)
    return (
        <div className="my-2 rounded-md border border-yellow-500/30 bg-yellow-500/10 p-2">
            <button
                onClick={() => setIsOpen(!isOpen)}
                className="flex w-full items-center gap-2 text-xs font-semibold text-yellow-600 dark:text-yellow-400"
            >
                <span>{isOpen ? '▼' : '▶'}</span>
