...
**Goal:** Effectiva's agent system helps students plan and balance study, work, and life by coordinating multiple specialized agents (Study, Work, Life, Scheduling) through a central coordinator.[1]
...
- **Coordinator Agent**: Central orchestrator that manages communication between specialized agents and ensures balanced decision-making across study, work, and life domains.
- **Study Agent**: Handles academic planning, study schedules, exam preparation, and learning objectives.
- **Work Agent**: Manages professional tasks, career goals, job-related responsibilities, and work-life balance.
- **Life Agent**: Focuses on personal well-being, hobbies, relationships, health, and leisure activities.
- **Scheduling Agent**: Coordinates calendars, deadlines, appointments, and time allocation across all domains.
...
```
|__ agents/          # Specialized agent implementations
|_____ base.py       # Base agent class
|_____ coordinator.py # Central coordinator agent
|_____ study_agent.py # Study planning agent
|_____ work_agent.py  # Work management agent
|_____ life_agent.py  # Life balance agent
|_____ scheduling_agent.py # Scheduling agent
|_____ reasoning.py  # Reasoning utilities
|_____ prompts.py    # Agent prompt management
|__ app/             # Main application code
|__ config/          # Configuration settings
|__ docs/            # Documentation
|__ dspy_programs/   # DSPy program implementations
|__ examples/        # Example usage scripts
|__ integrations/    # External integrations (e.g., WhatsApp)
|__ memory/          # Memory management
|__ prompts/         # Versioned prompt files (YAML)
|_____ *.yaml
|__ schemas/         # Data schemas and models
|__ tests/
|_____ evaluations/  # Jupyter notebooks for component evaluation
|________ *.ipynb
|_____ scenarios/    # End-to-end scenario tests
|________ *.test.py
|__ tools/           # Agent tools and utilities
|__ ui/              # Next.js frontend application
